                ...
                if (AmongUsClient.Instance.AmHost) {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                        ...
                        if (dummyComponent != null && dummyComponent.enabled)
                            ...
                            blockStart = true;
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 是其他或者无版本号的 The Other Roles\n</color>";
                        } else {
                            PlayerVersion PV = playerVersions[client.Id];
                            int diff = TheOtherRolesPlugin.Version.CompareTo(PV.version);
                            if (diff > 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName}是旧版本的 The Other Roles (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
                            } else if (diff < 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 有新版本的 The Other Roles (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
