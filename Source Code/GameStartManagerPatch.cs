                ...
                if (AmongUsClient.Instance.AmHost) {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                        ...
                        if (dummyComponent != null && dummyComponent.enabled)
                            ...
                            blockStart = true;
                            message += $"<size=50%><color=#FF0000FF>{client.Character.Data.PlayerName} 可能没有安装 The Other Roles\n</color></size>";
                        } else {
                            PlayerVersion PV = playerVersions[client.Id];
                            int diff = TheOtherRolesPlugin.Version.CompareTo(PV.version);
                            if (diff > 0) {
                                message += $"<size=50%><color=#FF0000FF>{client.Character.Data.PlayerName}使用了旧版本的 The Other Roles (v{playerVersions[client.Id].version.ToString()})\n</color></size>";
                                blockStart = true;
                            } else if (diff < 0) {
                                message += $"<size=50%><color=#FF0000FF>{client.Character.Data.PlayerName} 使用了新版本的 The Other Roles (v{playerVersions[client.Id].version.ToString()})\n</color></size>";
                                blockStart = true;
                            }
                             //  else if (!PV.GuidMatches()) { // version presumably matches, check if Guid matches
                             // message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 有修订版本的 TOR v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()})</size>\n</color>";
                             // blockStart = true;
                             // }
                        }
                    }
